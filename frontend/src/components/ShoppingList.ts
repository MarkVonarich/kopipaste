...
export function ShoppingList(items: ShoppingItem[], readOnly: boolean, saving = false, confirmClear = false, error = ''): string {
  const activeItems = items.filter((item) => !item.completed);
  const completedItems = items.filter((item) => item.completed);
  const completed = completedItems.length;
  const rows = (values: ShoppingItem[]) => values.map((item) => `<div class="shopping-item ${item.completed ? 'completed' : ''}" data-id="${item.id}">
  ...
  return `<div class="shopping-list" data-testid="shopping-list">
    ${readOnly ? '<p class="caption">Список доступен только для чтения.</p>' : `<form class="shopping-add" data-action="shopping-add">
