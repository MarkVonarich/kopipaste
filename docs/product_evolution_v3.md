...
Report-ready What's New cards are user-specific candidates concatenated into the existing resolver before dismissal, TTL, family de-duplication, ranking, and the five-card limit are applied. Eligibility uses one bounded PostgreSQL aggregate with filtered counts over the latest completed week and month. Candidate IDs include the completed period end date, so dismissing one report does not suppress every future period; the release date is that period end, preserving the existing 21-day TTL. Typed weekly/monthly actions contain no URLs or executable payloads. The static `reports-2-0` release card uses family `reports` and the same typed Reports navigation. No Telegram push notification is added.
