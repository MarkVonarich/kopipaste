...
def build_export_xlsx(path: str, rows: list[dict], dfrom: date, dto: date, locale: str = "ru"):
    ...
    exp_sum = sum((to_decimal_money(r["amount"]) for r in exp), Decimal("0.00"))
    inc_sum = sum((to_decimal_money(r["amount"]) for r in inc), Decimal("0.00"))
    ...
    for r in exp: grp_exp[r["category"]] += to_decimal_money(r["amount"])
    ...
    for r in inc: grp_inc[r["category"]] += to_decimal_money(r["amount"])
    ...
    for r in sorted(rows, key=lambda x: (x["op_date"], x["id"])):
        ws2.append([r["op_date"], r["type"], r["category"], float(to_decimal_money(r["amount"])), export_comment(r.get("comment")), export_source_label(r.get("source"), locale), r["id"]])
    for c in ws2[1]:
        c.font = Font(bold=True); c.fill = PatternFill("solid", fgColor="D9E1F2")
    ws2.freeze_panes = "A2"
    ws2.auto_filter.ref = f"A1:G{max(1, ws2.max_row)}"
    for i in range(2, ws2.max_row + 1): ws2[f"D{i}"].number_format = '# ##0.00 "₽"'
    ...
    for r in rows:
        k = (r["type"], r["category"])
        grp[k]["sum"] += to_decimal_money(r["amount"]); grp[k]["count"] += 1
        type_totals[r["type"]] += to_decimal_money(r["amount"])
    for (tp, cat), st in sorted(grp.items(), key=lambda x: (x[0][0], -x[1]["sum"])):
        total = type_totals[tp] or 1
        ws3.append([tp, cat, float(st["sum"]), st["count"], round(float(st["sum"] * Decimal("100") / total), 2)])
    ...
    for r in rows:
        ...
        if r["type"] == "Расходы": mgrp[m]["e"] += to_decimal_money(r["amount"])
        if r["type"] == "Доходы": mgrp[m]["i"] += to_decimal_money(r["amount"])
        mgrp[m]["c"] += 1
    ...
    for r in rows:
        ...
        if r["type"] == "Расходы": wgrp[k]["e"] += to_decimal_money(r["amount"])
        if r["type"] == "Доходы": wgrp[k]["i"] += to_decimal_money(r["amount"])
