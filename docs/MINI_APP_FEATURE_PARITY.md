...
| Категории: выбор | Да | Да | `services.categories`, `services.category_preferences`; category APIs | Parity | UX polish | Personal priority/relevance affects suggestions while hidden categories remain manageable and historical data is unchanged. |
...
| Режим отпуска | Да | Да | `services.notification_preferences`, `services.automatic_notifications`; `/miniapp/api/profile/vacation` | Parity | Release validation | Proactive finance delivery pauses without backlog; explicit user reminders continue. |
...
| Удаление данных | Да | Да | `services.analytics_privacy.apply_account_deletion`, `services.personal_data_deletion`; profile privacy APIs | Parity | Release validation | History has preview plus destructive confirmation; account deletion additionally requires typed `УДАЛИТЬ` and ends in a terminal state. |
