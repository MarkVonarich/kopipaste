...
A period is valid only when it has coverage for the specific planning purpose in the requested effective currency. Category limits, general limits, and grouped budgets require at least one expense operation; an income-only period is omitted and never manufactured into zero spending. Goal comfortable cash flow requires at least one income operation and at least one expense operation in the same period; income-only and expense-only periods cannot establish `income - expense`. Within a valid spending period, zero target-category spend remains a factual zero because other expense activity proves expense tracking existed. Four valid coverage periods produce `good` confidence, two or three produce `limited`, and zero or one produce `insufficient`. The response includes each valid period, operation, income, and expense counts, income, expense, net, target amount, arithmetic mean, and confidence. Missing coverage is never inserted as zero. Spending recommendations are available for `good` or `limited` history; `insufficient` history has no suggested amount. Goal required pace remains independent of this historical coverage and is still calculated from target, current balance, deadline, and schedule.
...
## Reports 2.0 architecture

PR 4 adds Reports as a mode inside the existing Analytics destination, not as a new bottom-navigation tab. The report read model lives in `services/reports.py`. The Mini App API authenticates the request through the existing Analytics workspace and filter path, asks Analytics 2.0 for its bounded aggregate response, and lets the Reports service select and serialize one exact-currency financial story. Reports never reload full operation history, query once per category or merchant, or reconstruct totals in the browser.

Reports support three kinds. `selected` preserves the current global Analytics workspace, period, operation type, and category scope. `completed_week` selects the latest finished Monday-through-Sunday week and compares it with the preceding equal seven-day period. `completed_month` selects the latest finished calendar month and uses Analytics' calendar-month comparison semantics for the preceding complete month. Neither completed kind includes the current unfinished week or month.

Manual entry from Analytics carries the user's current Analytics currency into the selected-period report unless the user has already deliberately selected another report currency. Switching between selected, weekly, and monthly reports keeps requesting that report currency. The backend remains authoritative and safely reconciles the request to an available exact currency when the requested currency has no data in the destination scope.

The report summary contains backend-calculated income, expense, financial result, and operation count. Comparison metrics pass through Analytics' `zero_baseline`, `empty_previous`, and `sign_change` states without inventing percentages. Missing data is explicit: no-data reports have no manufactured summary, while income-only and expense-only reports state that the covered scope contains only that operation side. Deterministic observations are bounded to three and reuse the same summary, comparison, category-contribution, and merchant aggregates; there is no LLM, forecast, or advice language.

Every report selects one effective currency. Explicit operation currencies stay exact. Legacy operations with `currency IS NULL` enter only the user's validated profile/default-currency group through the existing Analytics `COALESCE(currency, profile_currency)` contract. Available currencies are exposed for explicit switching, and there is no combined monetary total, FX lookup, or conversion. The frontend formats backend values but does not sum, subtract, or derive authoritative financial amounts.

Expense structure reuses canonical category keys and Analytics' bounded top rows plus non-drillable `Остальные`. Merchant structure reuses Merchant Key V1, folded aliases, bounded top rows, and non-drillable `Остальные` and `Без описания`; Reports adds only a Decimal-safe average check from the already aggregated merchant total and count. Drilldown scopes carry the authorized workspace, exact dates, operation type, currency, canonical `category_key` or `merchant_key`, and any pre-existing category scope into Operations. Display labels are never authoritative identifiers. Before a report drilldown changes navigation state, the frontend stores an explicit transient snapshot of the report workspace, global filters, mode, kind, and currency. Telegram Back clears the temporary Operations scope, restores that snapshot, and reloads the same report without leaving drilldown dates or categories installed globally. Normal Operations navigation creates no such return context.

The existing XLSX implementation remains the only export flow. A selected-period report may open it only when its scope has at most one available currency, because the existing export can preserve workspace, dates, operation type, and category but cannot filter a single currency. Completed and mixed-currency reports show an unavailable state instead of exporting a different dataset.

Report-ready What's New cards are user-specific candidates concatenated into the existing resolver before dismissal, TTL, family de-duplication, ranking, and the five-card limit are applied. Eligibility uses one bounded PostgreSQL aggregate with filtered counts over the latest completed week and month. The query applies the same authorized workspace, operation type, and category scope that the typed action will open, so unrelated operations cannot make an empty scoped report appear ready. Category text remains only a SQL parameter and is never copied into candidate metadata or IDs. Candidate IDs include only the completed period end date, so dismissing one report does not suppress every future period; the release date is that period end, preserving the existing 21-day TTL. Visibility and dismissal validation use the same resolved user-local date, while valid week/month end shapes and `0 <= age < 21` remain enforced. Typed weekly/monthly actions contain no URLs or executable payloads. The static `reports-2-0` release card uses family `reports` and the same typed Reports navigation. No Telegram push notification is added.

Product events are limited to `report_opened`, `report_drilldown_opened`, and `report_export_requested`. Allowed properties are bounded report kind, period kind, workspace type, operation type, result/status, source, drill kind, and currency code. Amounts, category or merchant text, descriptions, search text, arbitrary IDs, and raw financial content are excluded.

Reports adds no persistence and requires no migration. It reuses authenticated Analytics queries, Merchant Intelligence grouping, announcement dismissal storage, and the existing operation/export routes. PR 5 remains deferred: Vacation Mode, category priority/relevance, and the broader privacy and behavior-control surface are not implemented here.
...
## Profile and behaviour controls

PR 5 keeps the existing Profile information architecture and adds `Поведение` and `Данные и приватность`. Home composition remains at Profile -> Home -> Home settings. No new bottom-navigation destination or account system is introduced.

Vacation Mode is a global user preference stored in `notification_preferences`. Its inclusive active interval is evaluated from the user's resolved local `DATE`: `vacation_start <= user_local_date <= vacation_end`. Vacation takes precedence over Quiet Hours for proactive financial delivery, but neither saved notification toggles nor Quiet Hours are rewritten. Daily nudges, plan and budget/limit signals, goal and recurring-spend signals, and weekly/monthly reports are skipped while active. Explicit user-created reminders and force-immediate administrative previews remain available. Pending eligible notifications are marked skipped when Vacation is enabled, and period-specific attempts created during Vacation are also skipped, so no stale backlog is delivered after it ends. Future normal periods resume under the unchanged notification and Quiet Hours preferences.

Personal category preferences use canonical `normalized_category_key()` identity plus authenticated user, nullable workspace (legacy-personal scope), and operation type. `high` priority changes presentation order and can break ties only after deterministic source/confidence precedence; it never defeats an exact alias or stronger learned result. `relevant=false` removes a category from new suggestion and selection candidates for that user and scope, while management, historical operations, Analytics, Reports, and totals remain unchanged. Editing an entity preserves its currently selected hidden category. Rename and transfer migrate preferences in the existing category transaction; a destination preference wins collisions. Shared-workspace identity changes update each member's own preference key without exposing preference values, while personal changes remain owner-scoped. Archive/delete removes obsolete preference rows, which never count as a category reference.

The privacy section reuses the existing XLSX flow under the precise label `Экспорт финансовых данных`. Financial-history deletion resolves supported relative periods from `user_local_date`, presents an exact preview for operations, drafts, related records, and all-history goals, then requires a separate destructive confirmation. Execution uses `delete_financial_history`; personal scope cannot remove another member's shared operations. Full account deletion presents a user-readable `dry_run_delete_user_data` summary, requires a second screen and exact typed `УДАЛИТЬ`, applies existing analytics privacy semantics, and invokes `delete_user_data`. Personal workspaces are removed; other members' shared content remains and retained attribution is anonymized. The frontend clears sensitive state and stops on a terminal close screen without emitting a post-delete event or bootstrapping a new user.

Migration `20260813_023_profile_behaviour_controls.sql` adds the three Vacation columns and `user_category_preferences`. It is additive and idempotent, uses user/workspace cascades consistent with existing deletion semantics, and is required before deploying PR 5 code. It is not applied by this pull request. The static `profile-controls-v1` What's New candidate uses family `profile-controls`, action `OPEN_PROFILE`, and planned `released_on=2026-08-13`. That date must be changed in the final release pass if first production visibility occurs later.

## Explicitly deferred

Product Evolution v3 does not add credit products, FX conversion, new Telegram group behavior, LLM recommendations, bank integrations, remote announcement content, or a second notification/category/privacy engine.
