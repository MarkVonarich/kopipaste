...
class MiniAppAPI:
    ...
    def operations(self, req: MiniAppRequest, params: dict[str, Any]) -> dict:
        ...
        if category_key:
            if len(category_key) > 80:
                raise MiniAppError(400, "bad_category", "Invalid category.")
            filters.append("LOWER(TRIM(REGEXP_REPLACE(COALESCE(o.category,''), '\\s+', ' ', 'g')))=%s")
            values.append(category_key.casefold())
    ...
    def analytics(self, req: MiniAppRequest, params: dict[str, Any]) -> dict:
        ...
        response_currencies = sorted(set(available_currencies) | set(chart_currencies))
        currency_groups = {
            currency: {
                "summary": {
    ...
    def _metric_comparison(self, current: Decimal, previous: Decimal) -> dict[str, Any]:
        ...
        if previous == 0:
            ...
            pct = (abs(delta) / previous * Decimal("100")).quantize(Decimal("0.01"))
        ...
        *,
        ...
        for currency in currency_codes:
            ...
            result[currency] = {
                ...
                "result": self._metric_comparison(cur_income - cur_expense, prev_income - prev_expense),
    ...
    def _change_contribution(
        ...
        *,
        ...
        structure = self._dimension_structure(req, tx, prev_tx, op_type, dimension="category", currencies=currencies, top_n=READ_PAGE_LIMIT)
        groups = {}
        for currency, group in structure["currency_groups"].items():
            items = sorted(group["items"], key=lambda item: (-abs(to_decimal_money(item["delta"])), str(item["category"])))[:CHART_TOP_N]
            total_current = sum((to_decimal_money(item["total"]) for item in group["items"]), Decimal("0.00"))
            total_previous = sum((to_decimal_money(item["previous_total"]) for item in group["items"]), Decimal("0.00"))
            ...
            groups[currency] = {
                ...
                "items": items,
                "reconciles": sum((to_decimal_money(item["delta"]) for item in group["items"]), Decimal("0.00")) == total_delta,
            }
    ...
    def _category_merchant_breakdown(self, req: MiniAppRequest, tx: TransactionFilters, op_type: str, currency: str, category_key: str) -> dict:
        rows = pg_fetchall(
            ...
            SELECT NULLIF(TRIM(COALESCE(comment,'')), ''), COALESCE(SUM(amount),0), COUNT(*)
             ...
             WHERE {tx.where_sql}
               AND type=%s
               AND COALESCE(currency, %s)=%s
               AND LOWER(TRIM(REGEXP_REPLACE(COALESCE(category,''), '\\s+', ' ', 'g')))=%s
             GROUP BY NULLIF(TRIM(COALESCE(comment,'')), '')
             ORDER BY COALESCE(SUM(amount),0) DESC, NULLIF(TRIM(COALESCE(comment,'')), '')
             LIMIT %s
            """,
            (*tx.params, op_type, get_user_currency(req.user_id), currency, category_key.casefold(), CHART_TOP_N),
        )
        ...
        return {"currency": currency, "total": total, "items": items}
    ...
    def _analytics_detail(self, req: MiniAppRequest, tx: TransactionFilters, prev_tx: TransactionFilters, params: dict[str, Any], op_type: str) -> dict | None:
        ...
        if kind == "category":
            ...
            return {
                ...
                "visible_total": merchant_breakdown["total"],
        ...
        total = sum((to_decimal_money(op["amount"]) for op in operation_items), Decimal("0.00"))
    ...
    def _analytics_search(self, req: MiniAppRequest, tx: TransactionFilters, params: dict[str, Any], op_type: str, *, currencies: list[str] | None = None) -> dict:
        ...
        q = f"%{query[:80]}%"
        currency_filter = ""
        values: list[Any] = [*tx.params, op_type]
        selected_currency = str(params.get("currency") or "").strip().upper()
        if selected_currency:
            currency_filter = "AND COALESCE(currency, %s)=%s"
            values.extend([get_user_currency(req.user_id), selected_currency])
        rows = pg_fetchall(
            f"""
            SELECT category, NULLIF(TRIM(COALESCE(comment,'')), ''), COALESCE(currency, %s),
                   COALESCE(SUM(amount),0), COUNT(*), MAX(id)
              FROM public.operations
             WHERE {tx.where_sql}
               AND type=%s
               {currency_filter}
               AND (category ILIKE %s OR comment ILIKE %s)
             GROUP BY category, NULLIF(TRIM(COALESCE(comment,'')), ''), COALESCE(currency, %s)
             ORDER BY COUNT(*) DESC, COALESCE(SUM(amount),0) DESC
             LIMIT 20
            """,
            (get_user_currency(req.user_id), *values, q, q, get_user_currency(req.user_id)),
        )
        items = []
        seen = set()
        for category, merchant, currency, total, count, operation_id in rows:
            currency_code = str(currency)
            if currencies and currency_code not in currencies:
                continue
            category_name = str(category or "Прочее")
            category_key = normalized_category_key(category_name)
            cat_token = ("category", category_key, currency_code)
            if cat_token not in seen and query.casefold() in category_name.casefold():
                seen.add(cat_token)
                items.append({
                    "kind": "category",
                    "title": category_name,
                    ...
                    "params": {"detail_kind": "category", "detail_value": category_name, "detail_currency": currency_code},
                })
            merchant_name = str(merchant or "").strip()
            merchant_token = ("merchant", merchant_name, currency_code)
            if merchant_name and merchant_token not in seen and query.casefold() in merchant_name.casefold():
                seen.add(merchant_token)
                items.append({
                    "kind": "merchant",
                    "title": merchant_name,
                    "subtitle": f"{int(count or 0)} операций",
