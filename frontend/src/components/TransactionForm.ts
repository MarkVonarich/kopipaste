...
export function TransactionForm(options: CategoryOption[], params: {
  ...
  return `
    ${params.error ? `<div class="error" data-state="save-error">${esc(params.error)}</div>` : ''}
    <form class="form-grid" data-action="${params.action}" ${params.id ? `data-id="${params.id}"` : ''}>
      <input type="hidden" name="type" value="${params.type}" />
      <label>Сумма<input class="input" name="amount" inputmode="decimal" value="${esc(op?.amount || '')}" placeholder="0,00" required ${params.saving ? 'disabled' : ''} /></label>
      <label>Категория
      ...
      <label>Описание<textarea class="textarea" name="description" maxlength="200" required ${params.saving ? 'disabled' : ''}>${esc(op?.description || '')}</textarea></label>
      <label>Дата<input class="input" name="op_date" type="date" value="${esc(op?.op_date || new Date().toISOString().slice(0, 10))}" required ${params.saving ? 'disabled' : ''} /></label>
